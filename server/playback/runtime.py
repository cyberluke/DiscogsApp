...
class PlaybackRuntime:
    def __init__(
        ...
        playback_start_offset_seconds: float = 1,
    ...
    def observe_track(self, track: Any) -> dict[str, Any]:
        with self._lock:
            ...
            self._state.started_at = time.monotonic() + self.playback_start_offset_seconds
    ...
    def resume(self, send_hardware: bool = True) -> dict[str, Any]:
        with self._lock:
            ...
            if self._state.playback_state in {PLAYBACK_PAUSED, PLAYBACK_STOPPED} and self._state.current_track:
                resume_elapsed = self._state.paused_elapsed if self._state.playback_state == PLAYBACK_PAUSED else self._state.elapsed
                self._state.started_at = time.monotonic() - resume_elapsed
                self._state.playback_state = PLAYBACK_PLAYING
                self._state.load_delay_seconds = 0
                self._state.load_delay_remaining = 0
    ...
    def handle_transport_status(self, status: str, payload: Mapping[str, Any] | None = None) -> dict[str, Any]:
        ...
        if normalized_status == 'PLAY':
            return self.resume(send_hardware=False)
    ...
    def _start_current_track_locked(self, send_hardware: bool = True):
        ...
        load_delay = self._changer_load_delay_seconds(target_cd)
        ...
        self._state.started_at = time.monotonic() + load_delay + self.playback_start_offset_seconds
        self._state.paused_elapsed = 0
        self._state.load_delay_seconds = load_delay
        self._state.load_delay_remaining = load_delay
        self._state.playback_state = PLAYBACK_PLAYING
        self._state.error = None
    ...
    def _update_progress_locked(self):
        ...
        now = time.monotonic()
        mechanical_ready_at = self._state.started_at - self.playback_start_offset_seconds
